...
from app.utils import compute_summary, compute_summary2D, compute_error_prediction
...
def summary(
    ...
    description.markdown(
        ...
        of VIS magnitude $\mathcal{B}$ (see Eqs. 5 and 4 of the paper). The second
    ...
    for param in params:
        try:
            results = compute_summary2D(
                df, params, codes, mag_bins, bt_bins, outlier_threshold
            )
...
def trumpet(df, params, codes, x_range, nb_bins, outlier_threshold, ymax):
    """
    Compute and plot the Summary Figure representing the different metrics (Bias, Dispersion and Outlier fraction)
    for different codes and parameters.
...
def score(
    ...
    print("snr in score", linear_SNR_weight)
